...
## Đợt 6: ngân hàng bài tự luận (essay + rubric) cho 6 chuyên đề "phương pháp giải đặc biệt" (theo yêu cầu trực tiếp của người dùng, 2026-08-14)

Trước đợt này, chỉ TD-06 (chuyên đề thuần "lập luận/chứng minh") có bài `essay` (9 bài, kèm `modelSolution` + rubric). Sáu chuyên đề còn lại trong nhóm "phương pháp giải đặc biệt" mà URD liệt kê riêng (Mục 4.3) — DH-08 (chuyển động nâng cao), DH-11 (giả thiết tạm), DH-12 (tính ngược từ cuối), DH-16 (phương pháp khử), HH-09 (tỉ số diện tích tam giác), HH-10 (cắt ghép hình) — chỉ có bài điền đáp số/trắc nghiệm, dù bản chất các phương pháp này đặt trọng tâm vào việc **trình bày được lập luận**, không chỉ ra đúng con số.

**Phạm vi**: +2 bài tự luận cho mỗi chuyên đề trong 6 chuyên đề trên = **12 bài mới**, file `src/content/exercises/essay-priority.ts`, nối vào `content/exercises/index.ts`. Không bổ sung thêm cho TD-06 vì ngân hàng đã khá đầy đủ (9 bài) từ trước. Mỗi bài theo đúng khuôn mẫu đã có ở TD-06: `solutionSteps` (2–3 bước) + `essay.modelSolution` (đoạn văn liền mạch) + `essay.rubric` (3 tiêu chí × 1 điểm).

- Nội dung 12 bài bám sát đúng tên phương pháp của từng chuyên đề (không chỉ đổi số liệu mà đổi cả yêu cầu đề bài từ "tính" sang "trình bày cách giải bằng phương pháp X"): DH-08 (ca-nô xuôi/ngược dòng tìm vận tốc nước; nghỉ giữa đường không đổi vận tốc), DH-11 (gà–chó; vé người lớn–trẻ em), DH-12 (dãy phép tính tính ngược; tiền tiêu dần qua 2 ngày), DH-16 (vở–bút; cam–xoài, khử đại lượng chung), HH-09 (tam giác chung đáy do trung điểm; chung chiều cao với tỉ lệ đáy 2:1), HH-10 (đường chéo chia đôi hình chữ nhật; 4 tam giác góc trừ khỏi hình vuông).
- Tổng ngân hàng: 641→**653 bài** (561 numeric, 69 mcq, 23 essay).
- Rà soát lớp 2: 2 agent con độc lập (nhóm 1: 8 bài DH; nhóm 2: 4 bài HH), tự giải lại từ đầu. Vì cả 12 bài đều dạng tự luận (không có `acceptedValues` để đối chiếu tự động), đối chiếu hoàn toàn thủ công giữa kết luận + lập luận độc lập với `modelSolution`/`rubric` đã lưu — **12/12 khớp hoàn toàn, 0 điểm lệch.** Chi tiết: [`docs/content-review-log.md`](../content-review-log.md). Agent tự ghi chú 2 điểm chất lượng đề (không phải lỗi): DH-08-EX14 cố ý thiếu dữ kiện quãng đường/vận tốc vì đáp số không phụ thuộc; DH-16-EX9/EX10 nếu suy ngược giá đại lượng phụ (vở/xoài) thì không tròn số nhưng không ảnh hưởng vì đề chỉ hỏi giá đại lượng chính.
...
## Đợt 8 (phần 1/2): Sao lưu/khôi phục, ô nháp, âm thanh phản hồi, đọc đề giọng nói, thử thách tốc độ (theo yêu cầu trực tiếp của người dùng, 2026-08-14)

Đợt 8 gốc gồm 6 việc rất khác nhau về độ lớn (Sao lưu/khôi phục, ô nháp, âm thanh, đọc giọng nói, thử thách tốc độ, và Đồng bộ Firebase). Vì Đồng bộ Firebase đòi hỏi người dùng tự tạo project/tài khoản Firebase thật — không thể tự động hóa được — đã hỏi lại qua `AskUserQuestion` và người dùng chọn làm 5 việc nhẹ trước, để Firebase cho một lượt riêng (sẽ hướng dẫn từng bước lúc đó).

**1. Sao lưu & khôi phục (FR-H11) — làm trước tiên vì SY-02 yêu cầu đồng bộ Firebase sau này phải tái dùng đúng `exportAll()`/`importAll()` này, không xây API riêng.**
- `types/backup.ts`: `BackupData` (`version: 1`, `exportedAt`, cùng 6 danh sách tương ứng 6 bảng Dexie hiện có — không gồm nội dung bài học vì đó là dữ liệu tĩnh, không phải dữ liệu người dùng).
- `ProgressStore.exportAll()`/`importAll()`: `exportAll` gộp `getAttempts/getErrorLog/getTestResults/getProfile/listTopicProgress/listSessionOutcomes` đã có sẵn (không viết lại logic đọc); `importAll` chạy trong 1 transaction Dexie, xóa sạch rồi ghi đè từng bảng (khôi phục = ghi đè, không merge, đúng kỳ vọng người dùng khi chọn "khôi phục").
- `core/backup/validate-backup.ts` (`isValidBackupData`, hàm thuần, 5 unit test): kiểm tra cấu trúc tối thiểu của file JSON trước khi ghi đè — chặn trường hợp người dùng lỡ chọn nhầm file khác.
- `SettingsCard.tsx` (trang Hồ sơ): nút xuất file (tải `Blob` qua `<a download>`) và nút khôi phục (input file ẩn) kèm `window.confirm` cảnh báo ghi đè trước khi thực thi — đúng tinh thần SY-06 dù đây là bản cục bộ, chưa phải cloud.
- Xác nhận bằng Playwright vòng lặp thật: xuất file → đọc lại JSON đúng cấu trúc và đúng dữ liệu vừa tạo (kể cả 1 lượt làm bài thật) → khôi phục lại chính file đó → hiện đúng thông báo thành công, không lỗi console.

**2. Ô nháp (UX-09).** `components/ScratchPad.tsx` — textarea ẩn/hiện, state nội bộ hoàn toàn (không props, không lưu Dexie), gắn `key={`scratch-${current.id}`}` khi render trong `PracticeSession.tsx` để React tự mount lại (xóa nội dung nháp) mỗi khi sang câu mới, không cần tự viết logic reset.

**3. Âm thanh phản hồi (GM-07).** `core/sound/sound-effects.ts` — ngoại lệ duy nhất trong `core/` được phép chạm `AudioContext`/`localStorage` trực tiếp (ghi rõ trong docstring đầu file, vì phá quy ước "core = hàm thuần" mà dự án theo suốt). `playCorrectSound()` (2 nốt đi lên, vui tai) và `playIncorrectSound()` (180Hz, 180ms, sine — cố ý trầm và ngắn đúng yêu cầu URD "không chói tai"). Bật/tắt lưu ở `localStorage` (không cần đồng bộ đa thiết bị nên không đặt vào `LearnerProfile`). Gắn vào `PracticeSession.tsx` (đúng lúc chấm mcq/numeric) — **không** gắn vào `TestSession.tsx` vì màn hình Thi thử chỉ hiện kết quả ở cuối bài, không có phản hồi tức thời từng câu để phát âm thanh.

**4. Đọc đề bằng giọng nói (UX-15, UX-16).** `components/ReadAloudButton.tsx` — dùng `SpeechSynthesisUtterance` (`lang: 'vi-VN'`), tự ẩn hoàn toàn (`return null`) nếu `'speechSynthesis' in window` là `false` (UX-15). Effect cleanup phụ thuộc `[text]` vừa hủy giọng đọc khi đổi câu vừa hủy khi rời trang (UX-16) — không cần 2 effect riêng vì cleanup chạy ở cả hai trường hợp. Có hàm `toSpeakableText()` nhỏ bỏ cú pháp LaTeX thô (`$`, `\times`, `\frac{}{}`...) trước khi đọc, tránh giọng đọc phát âm ký hiệu vô nghĩa.

**5. Thử thách tốc độ (GM-08).** `SpeedChallenge.tsx` (route `/luyen-tap/thu-thach-toc-do`, liên kết từ `PracticeSetup`): chọn thời lượng 30/60/90s → đếm ngược, mỗi câu trả lời xong hiện chớp nhoáng đúng/sai 500ms (kèm âm thanh, tái dùng `core/sound`) rồi tự sang câu tiếp, không có gợi ý/lời giải (đúng tinh thần "rèn phản xạ", khác hẳn luồng học ở `PracticeSession`). `core/speed-challenge/select-pool.ts` lọc `level === 'basic'` + loại `essay` — dự án chưa migrate M1–M4 (xem Đợt 5) nên dùng `basic` làm ngưỡng gần đúng cho "M1–M2", có ghi chú rõ trong code. `core/speed-challenge/best-score.ts` so sánh kỷ lục, lưu ở `localStorage` (không phải Dexie). **Quyết định có chủ đích: không ghi các lượt trả lời trong chế độ này vào `Attempt`/sổ lỗi/mức thành thạo** — đây là chế độ rèn phản xạ tốc độ cao, ghi nhận chung với luyện tập bình thường sẽ làm nhiễu tín hiệu `mastery-engine` (vốn giả định mỗi lượt làm phản ánh mức hiểu thật, không phải bấm vội để kịp giờ).

- `npx tsc -b --noEmit` sạch, `npx vitest run` 121/121 xanh (+10 test mới: 5 `validate-backup`, 3 `best-score`, 2 `select-pool`).
