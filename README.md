...
- Lõi thuật toán (`core/`): `answer-checker` (chấm đáp số, FR-M05→M14), `error-log`, `topic-progress`, `test-generator` (sinh đề theo tỷ trọng), `mastery-engine` (tính mức thành thạo có trọng số + gợi ý lộ trình), `schedule` (sinh lịch học thích ứng) — toàn bộ là hàm thuần, không I/O, có unit test riêng.
- Nội dung: đủ **57/67 chuyên đề** theo URD v2.0 (6 nhóm SH/PS/DH/HH/DL/TD) với lý thuyết + ví dụ + kiểm tra nhanh, và **470 bài tập** (410 điền đáp số, 51 trắc nghiệm, 9 tự luận). 14 chuyên đề trọng điểm có 12 bài/chuyên đề; 43 chuyên đề còn lại hiện có 6–8 bài/chuyên đề. **Còn thiếu 10 chuyên đề mới của v2.0** (SH-11/12, PS-11, DH-15/16/17, HH-12, DL-07/08, TD-07) — xem phần "Còn thiếu so với URD v2.0" bên dưới.
...
- Kiểm chứng nội dung (Mục 18 URD v2.0): đã qua **lớp 1** (kiểm tra cấu trúc tự động) và **lớp 2** (giải lại độc lập bằng agent con tách biệt, đối chiếu tự động — 470/470 bài khớp, 0 điểm lệch). Chi tiết: [`docs/content-review-log.md`](./docs/content-review-log.md).
...
10 chuyên đề mới + thang 4 mức độ M1–M4 (hiện vẫn dùng `basic`/`advanced`), Gamification (xu/huy hiệu/âm thanh), Đồng bộ đa thiết bị qua Firebase, UI/UX Mục 11 còn lại (thanh tab đáy di động, phím tắt, đọc đề bằng giọng nói, bản in phụ huynh, layout 2 cột cho bài có hình/bảng), gợi ý sau khi làm sai (hint) và trang phân tích lỗi theo loại lỗi. Stack công nghệ **giữ nguyên** React 18 + CSS thuần + Dexie/IndexedDB (không nâng cấp Tailwind v4/React 19/localStorage như URD v2.0 "chốt" — quyết định có chủ đích, xem ADR).
